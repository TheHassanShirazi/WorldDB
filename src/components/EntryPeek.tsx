...
/**
 * The peek: a glance at an entry without leaving the graph.
 *
 * Deliberately shallow, and that discipline is what keeps ADR-0005's three
 * levels from collapsing into two surfaces fighting over one screen. It shows
 * enough to decide whether to open the entry — no more. Wanting the full body
 * here is the signal that the peek should not exist at all.
 */

import Link from "next/link";
import { ENTRY_TYPES } from "@/domain";
import type { ApiEntry } from "@/graphql/world";

const TEASER = 150;

export function EntryPeek({
  entry,
  onNavigate,
  onClose,
}: {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        <h3 className="mt-5 mb-1.5 text-[11px] uppercase tracking-[0.14em] text-white/40">
          Connected · {entry.connections.length}
        </h3>
      ...
      <footer className="border-t border-white/10 p-3">
        <Link
          href={`/entry/${entry.id}`}
          className="block w-full rounded-md border border-white/15 bg-white/5 px-3 py-2 text-center text-sm text-white/85 transition hover:bg-white/12 hover:text-white"
        >
          Open full entry →
        </Link>
      </footer>
